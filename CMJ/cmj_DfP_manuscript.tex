...
% We begin with a review of approaches to, and motivations for, designing novel DMIs. In particular, we consider methods for early stages of ideation and innovation, including user-centered and participatory design approaches drawn from HCI and oriented towards applications for creative practice. The Design for Performance workshops are then introduced, where  expert musicians built non-functional prototypes of imaginary DMIs. Section \ref{sec:results} reports the initial results of the in-workshop activities, and Section \ref{sec:thematic-analysis} provides the results of the post-workshop analysis that was conducted, which led to the generation of a list of design specifications for new instruments. Finally, Section \ref{sec:discussion} discusses the potential efficacy of the design fiction approach towards developing instruments that are viable for uptake and long-term use by expert musicians in real-world performance, and outlines future work for robust evaluation of the methods explored. 
...
%     \label{fig:malloch-model-of-interaction}
% \end{figure}
...
% An instrument's sound is typically generated and controlled through the various performative actions of its owner.
...
% what methods are effective in the development of novel DMIs that will make them appealing for use in artistic practice?
...
% UCD is less of a method in and of itself than a high level guideline under which several design strategies fall such as those in \citet{greenberg2011sketching}. 
...
% \label{sec:novel-approaches-to-idea-generation-and-prototyping}
...
%     \end{subfigure}
%     \caption{Probatio prototype constructed from base, structural support and multiple control modules (a); morphological chart suggesting new prototypes by combining features of existing instruments (b) \citep{Calegario2020}.}
% \end{figure}
...
% The next section reports on our own workshop that was styled after the Magic Machines workshop.
...
% Table \ref{tab:workshop-activities} provides a side by side comparison of the the Design for Performance and Magic Machine workshop activities (in particular Andersen's musical instrument design workshops \citep*[pp. 30-53]{Andersen2017}) and notes variations between them. In Section \ref{sec:workshop-activities} we describe each activity in detail.
...
% \label{sec:research-ethics-and-participant-consent}
...
% \label{sec:criteria-and-recruitment}
...
% Profiles of each participant are included in Appendix 1, including information gathered from the prescreening questionnaire and additional information provided from self-introductions at the beginning of the sessions. 
The workshop activities and procedures were reviewed and approved by the Research Ethics Board Office of \anonymize{McGill University} and all participants read and signed a form of informed consent before beginning. 
...
A schedule for the workshop had been developed based on Andersen's recommendations to run the workshop at a quick pace and keep a tight timeline. This was found to be an effective strategy to alleviate any potential anxieties or fears of failure that participants could experience during the creative and open-ended design activities \citep{Andersen2017}. The sessions were held on consecutive days in a spacious and well lit conference room at the \anonymize{Centre for Interdisciplinary Research in Music Media and Technology (CIRMMT) at McGill Universtity}. Tables were arranged together so that participants sat around the outside facing each other. A variety of crafting materials was  arranged on a separate table and covered with a cloth when the participants arrived. A videocamera was set up to record the sessions.
...
% The participants were initially given 25 minutes to complete the activity (pictured in Figure \ref{fig:instrument-building}), though extra time could be allotted if desired. In both sessions, five extra minutes were added, making the total length of the activity 30 minutes. 
...
% They began with a description of the music they had drawn, then described and demonstrated their instrument. 
...
% This exercise was less oriented around ranking of essential design elements and more concerned with facilitating a discussion around individual and shared priorities for bringing a new instrument to life.
...
% The voting activity was completed quickly and the workshop moved on to a final discussion.
...
% Given the abstract nature of the ``draw the music'' task and its express purpose to provide an on-ramp and context for the creative activity to follow. they it is unnecessary to discuss the output of the cards themselves and instead focus on the instruments that the participants created. 

% \begin{figure}[htbp]
%     \centering
%     \begin{subfigure}{1\textwidth}
%         \centering
%         \includegraphics[width=1\textwidth]{dfp_presentations_A.jpg}
%         \caption{Session A: P01--P03 (left to right)}
%         \label{fig:presentations_A}
%     \end{subfigure}
%     \par\bigskip
%     \begin{subfigure}{1\textwidth}
%         \centering
%         \includegraphics[width=1\textwidth]{dfp_presentations_B.jpg}
%         \caption{Session B: P04--P10 (clockwise from top left)}
%         \label{fig:presentations_B}
%     \end{subfigure}
...
% (As previously mentioned, P09 and the first author are active collaborators whose work is the subject of the following chapter.)
...
% Furthermore, the different sizes of the two groups produced highly varied results. For the smaller A group, the presentations were conversational and highly interactive. There was more agreement on key elements between the participants, which tended to be more abstract and less feature-driven (highlighted in the discussion that followed). By contrast, Session B contributed a wide diversity 
...
% We used an inductive approach, similar to that found in grounded theory \citep{Strauss1994} in which the data set is coded from the bottom up.
...
% The most common theme across both groups was the importance of physical and material properties of instruments (Theme 1). These aspects highlight the essential embodied connection between performer and instrument (\citet{Leman2008}, for example) as well as material-oriented interactions as discussed by \citet{Mudd2019}. In both sessions, participants highlighted the unique performance possibilities that could come through the use of different textures, shapes, materials and movements. Novel interface elements were often mixed with conventional input controls found on existing instruments (Theme 4) to form new hybrid instrument designs. 

% Flexibility and versatility are prioritized in different ways. Theme 2 emphasizes the ability for the performer to easily route audio and control signals in various complex ways, while Theme 5 highlights instruments that can be used in different ways and contexts, and accommodate a variety of different playing techniques and styles. 

% Theme 3 focuses on sound production and processing and overlaps with flexible signal routing (Theme 2) and versatility (Theme 5). Participants value powerful and dynamic instruments that provide a wide range of audio synthesis and modulation possibilities. Theme 1 is echoed here as well, as novel interactions are used as creative input controls. 
...
% By bringing in expert musicians, we aimed to leverage their tacit knowledge and experience of real-world performance practice in hopes that their input could direct the design of instruments that would be appealing and viable to be taken up into musical practice. 
...
% of implementing their designs into functional instruments. 
...
% A large part of the methodology designed for this project was adopted from Andersen's Magic Machine workshop playbook. 
...
% Our approach follows the same progression but over a longer time frame (including the Design for Performance workshop and subsequent instrument designs), with an interest in producing high fidelity prototypes suitable for use in artistic practice.
...
% The workshop design has adapted from Andersen's Magic Machine Workshops ~\citep{Andersen2017} and is informed by theories for DMI development based on previous design frameworks and HCI literature. 
...
\anonymize{We would like to extend our thanks to Collin Wang, who served as the assistant for the workshops. Thanks also to the Centre for Interdisciplinary Research in Music Media and Technology for the generous funding and facilities to carry out this research.}
